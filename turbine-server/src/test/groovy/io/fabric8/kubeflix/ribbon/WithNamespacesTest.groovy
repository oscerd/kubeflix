...
@WebIntegrationTest(
        [       "server.port:0",
                ...
                "turbine.aggregator.clusters.mycluster=ns1.*,ns2.*"
        ])
class WithNamespacesTest extends Specification {
    ...
    def setupSpec() {
        ...
        mockServer.expect().get()
                .withPath("/api/v1/namespaces/ns1/endpoints?labelSelector=hystrix.enabled%3Dtrue")
                .andReturn(200, new EndpointsListBuilder().withItems(
                        newEndpoint("service1", "ns1", "ip1"),
                        newEndpoint("service2", "ns1", "ip2"),
                    ).build())
                .always()

        mockServer.expect().get()
                .withPath("/api/v1/namespaces/ns2/endpoints?labelSelector=hystrix.enabled%3Dtrue")
                .andReturn(200, new EndpointsListBuilder().withItems(
                        newEndpoint("service3", "ns2", "ip3"),
                        newEndpoint("service4", "ns2", "ip4")
                    ).build())
                .always()
    ...
    def "when multiple namespaces are configured discovery should include all hystrix.enabled labeled endpoints in the specified namespaces"() {
        given:
